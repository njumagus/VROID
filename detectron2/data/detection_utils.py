...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    class_ids = [obj["class_id"] for obj in annos]
    class_ids = torch.tensor(class_ids, dtype=torch.int64)
    target.gt_class_ids = class_ids
    if len(annos) and "labeled" in annos[0]:
        labeled_flags=[obj["labeled"] for obj in annos]
        target.gt_labeled = labeled_flags
...
def triplets_to_triplets(triplets_list, image_size):
    ...
    gt_subject_ids = torch.tensor([triplet['subject_id'] for triplet in triplets_list], dtype=torch.int64)
    gt_object_ids = torch.tensor([triplet['object_id'] for triplet in triplets_list], dtype=torch.int64)
    gt_relation_ids = torch.tensor([triplet['relation_id'] for triplet in triplets_list], dtype=torch.int64)

    target.gt_subject_ids = gt_subject_ids
    target.gt_object_ids = gt_object_ids
    target.gt_relation_ids = gt_relation_ids
